...
class FakeDatabase {
  ...
  async getAllAsync<T>(source: string, ...params: unknown[]): Promise<T[]> {
      ...
      return this.state.types
        .map((type) => {
          const rule = this.state.rules
            .filter((candidate) => candidate.milkTypeId === type.id && candidate.effectiveFrom <= date)
    ...
    if (source.includes('FROM delivery_rules')) {
      return this.state.rules
        .slice()
        .sort((a, b) => b.effectiveFrom.localeCompare(a.effectiveFrom) || a.milkTypeId.localeCompare(b.milkTypeId))
        .map((rule) => ({
  ...
  async withExclusiveTransactionAsync(callback: (transaction: FakeDatabase) => Promise<void>): Promise<void> {
    await callback(this);
  }
}
...
export async function runMilkDatabaseTests(): Promise<void> {
  ...
  const baseRules: DeliveryRule[] = [
    { milkTypeId: 'cow', effectiveFrom: '2026-01-01', quantityMl: 2000, pricePaisePerLitre: 7000, enabled: true },
    { milkTypeId: 'buffalo', effectiveFrom: '2026-01-01', quantityMl: 1000, pricePaisePerLitre: 9000, enabled: true },
  ];
  ...
  assert(JanuaryDay.lines.every((line) => line.source === 'default'), 'daily defaults report default source');
  ...
  assertEqual(editedDay.lines.find((line) => line.milkTypeId === 'cow')?.source, 'override', 'override source');

  await clearOverride(db, '2026-01-10', 'cow');
  assertEqual((await getDeliveryDay(db, '2026-01-10')).totalQuantityMl, 3000, 'clearing override restores default');
  ...
  assertEqual((await getDeliveryDay(db, '2026-02-10')).lines.length, 1, 'disabled milk type is omitted');
